...
#include <linux/swapops.h>		/* swp_entry_t, ...		*/
 ...
 */
static inline void do_user_addr_fault_profiling(struct pt_regs *regs,
						unsigned long error_code,
						unsigned long address,
						int *adc_pf_bits,
						uint64_t pf_breakdown[])
{
	...
	if (is_vsyscall_vaddr(address)) {
		if (emulate_vsyscall(error_code, regs, address)) {
			uint64_t pf_ts = pf_cycles_end();
			adc_pf_breakdown_end(pf_breakdown, ADC_LOCK_GET_PTE,
					     pf_ts);
	 ...
	 */
	skip_mm_lock = hermit_mm_lock_skippable(mm, address);
	if (skip_mm_lock) {
		locked = true;
		adc_profile_counter_inc(ADC_HERMIT_VPAGES);
	}
	else
...
retry:
		...
		adc_pf_breakdown_stt(pf_breakdown, ADC_LOCK_GET_PTE, pf_ts);
		if (!skip_mm_lock)
			mmap_read_lock(mm);
	} else {
		/*
...
good_area:
	 ...
	 */
	fault = handle_mm_fault_profiling(vma, address, flags, regs,
	...
	if (unlikely((fault & VM_FAULT_RETRY) &&
		...
		adc_pf_breakdown_end(pf_breakdown, ADC_SET_PAGEMAP_UNLOCK,
				     pf_cycles_end());
		goto retry;
	}
...
NOKPROBE_SYMBOL(do_user_addr_fault_profiling);
...
void do_user_addr_fault(struct pt_regs *regs,
			...
			unsigned long address)
{
	return do_user_addr_fault_profiling(regs, error_code, address, NULL,
					    NULL);
}
...
static __always_inline void
handle_page_fault_profiling(struct pt_regs *regs, unsigned long error_code,
			    unsigned long address, int *adc_pf_bits,
			    uint64_t pf_breakdown[])
{
	...
	if (unlikely(fault_in_kernel_space(address))) {
		...
		do_user_addr_fault_profiling(regs, error_code, address,
					     adc_pf_bits, pf_breakdown);
		/*
...
handle_page_fault(struct pt_regs *regs, unsigned long error_code,
			    ...
			    uint64_t pf_breakdown[])
{
	return handle_page_fault_profiling(regs, error_code, address, NULL,
					   NULL);
}
