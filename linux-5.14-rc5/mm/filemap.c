...
void delete_from_page_cache_lite(struct page *page, void *shadow)
{
	struct address_space *mapping = page->mapping;
	unsigned long flags;
...
/* [Hermit]
 * The implementation is the same as __lock_page_or_retry() but skipped the
 * mmap_read_unlock(). Used for optimisitic swap fault case where we omitted
 * the mmap_read_lock().
 */
int __lock_page_or_retry_skip_mmlock(struct page *page, struct mm_struct *mm,
			 unsigned int flags)
{
	if (fault_flag_allow_retry_first(flags)) {
		/*
