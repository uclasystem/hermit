...
/**
 * RDMA connection initialization
 */
int main(int argc, char *argv[]) {
  ...
  if (argc < 5) {
    printf(
        "Usage: %s <ip> <port> <memory size in GB> <#(cores) on CPU server>\n",
        argv[0]);
    exit(-1);
  }
 ...
 *  1) Meta data Region. Maybe not commit all the available size. Not exceed the
 * REGION_SIZE_GB. Only commited space can be registered as RDMA buffer. 2) Data
 * Region. Aligned at REGION_SIZE_GB.
 *
...
/**
 * Get a cm_event : RDMA_CM_EVENT_CONNECT_REQUEST, response it.
 */
...
/**
 * Already intialized hardware information : cm_id,  communication
 * event_channel, listen, ip:port. Build the RDMA connection required
 * structures: RDMA QP, CQ, RDMA messages buffer and a daemon thread, poll_cq,
 * wait for events.
 */
...
/**
 * Build a daemon thread,poll_cq(void *ctx), to handle the 2-sided RDMA
 * communication. The 2-sided RDMA message is used to build the connection with
 * CPU server. After this, CPU server uses 1-sided RDMA read/write to access the
 * memory pool in current Memory server.
 */
...
/**
 * Post a receive WR and wait for RDMA message.
 */
...
/* 2-sided RDMA message to CPU server */
  ...
  if (rdma_session->connected == 0) {
    ...
    for (i = 0; i < rdma_session->mem_pool->region_num; i++) {
      ...
      if (rdma_session->mem_pool->mr_buffer[i] != NULL) {
        print_debug(
            stderr,
            "Register Region[%d] : 0x%llx to RDMA Buffer[%d] : 0x%llx, rkey: "
...
/**
 * Post a two-sided RDMA message to client to inform the free memory size, GB,
 * in server.
 */
